...
async def extract_interactive_elements(page):
    ...
    return await page.evaluate(js_code, BID_ATTR)


async def highlight_elements(page, elements, max_retries=3, retry_delay=1000):
    js_code = """
    (elements) => {
    ...
    for attempt in range(max_retries):
        highlighted_count = await page.evaluate(js_code, elements)
    ...
    logger.info(
        f"Warning: Only {highlighted_count}/{len(elements)} elements were highlighted after {max_retries} attempts.")
